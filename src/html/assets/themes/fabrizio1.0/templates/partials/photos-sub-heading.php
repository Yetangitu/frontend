<small>
  (
    ...
    sort:
    <?php if($sortParts[0] == 'date_taken') { ?>
      <i class="icon-calendar" title="Sorted by date taken"></i>
    <?php } else { ?>
      <a href="?sortBy=<?php $this->utility->safe($this->utility->getSortByParams('by','date_taken',$currentSortBy, false)); ?>" title="Sort by date date taken"><i class="icon-calendar"></i></a>
    <?php } ?>
    or
    <?php if($sortParts[0] == 'date_uploaded') { ?>
      <i class="icon-upload" title="Sorted by date uploaded"></i>
    <?php } else { ?>
      <a href="?sortBy=<?php $this->utility->safe($this->utility->getSortByParams('by','date_uploaded',$currentSortBy, false)); ?>" title="Sort by date uploaded"><i class="icon-upload"></i></a>
    <?php } ?>
