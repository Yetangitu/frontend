...
class DatabasePostgreSql implements DatabaseInterface
{
  ...
  /**
    ...
    *
    * @param string $element_id ID of the photo whose activity should be deleted
  ...
  public function getAlbum($id, $email, $validatedPermission = false)
  {
    ...
    if($album === false)
      return false;
    else if(!$validatedPermission && !$this->isAdmin() && $album['count_public'] == 0)
      return false;
  ...
  public function getAlbumByName($name, $email)
  {
    ...
    if($album === false)
      return false;
    else if((empty($email) || $this->owner !== $email || $this->getActor() !== $email) && $album['count_public'] == 0)
      return false;
...
/*    $photos = $this->db->all("SELECT pht.* 
  ...
  /**
    * Retrieve a credential by user_token
    *
    * @param string $user_token user_token of the credential to get
    * @return mixed Array on success, FALSE on failure
    */
  ...
  public function getPhotoNextPrevious($id, $filterOpts = null)
  {
    ...
    // remembering that the photos are sorted in descending order on date_sort_by_day
  ...
  public function getShareToken($id)
  {
    ...
    if($token['date_expires'] > 0 && $token['date_expires'] <= time())
    {
  ...
  public function getShareTokens($type = null, $data = null)
  {
    ...
    {
      if($value['date_expires'] > 0 && $value['date_expires'] <= time())
        unset($tokens[$key]);
    }
  ...
  /**
    ...
    * @param array $element_ids IDs of the elements to update.
    * @return boolean
    */
...
*/
      ...
      switch($name)
      {
        ...
        case 'sortBy':
          ...
          switch($value)
          {
            ...
            default:
              if($table === 'photo')
                $sortBy = 'ORDER BY date_sort_by_day DESC, date_taken ASC';
              else if($table === 'activity')
                $sortBy =  'ORDER BY date_created DESC';
          }
          /*
  ...
  /**
    * Builds the date_sort_by_day string from a timestamp
    *
    * @param string $time time to format
    * @return string
    */
  private function date_sort_by_day($time)
  {
  ...
  private function normalizePhoto($photo)
  {
    ...
    $photo['app_id'] = $this->config->application->app_id;
  ...
  private function preparePhoto($id, $params)
  {
    ...
    if(isset($params['date_taken']))
    {
      // here we seed a field for the default sort order of day descending and time ascending

      $bindings[':date_sort_by_day'] = $this->date_sort_by_day($params['date_taken']);
      $paramsOut['date_sort_by_day'] = ':date_sort_by_day';
    }
